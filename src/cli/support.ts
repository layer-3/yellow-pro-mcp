...
Setup:
  yellow-pro connect <pairing-code> [--client CLIENT] [--profile NAME] [--auth-url URL --api-url URL] [--replace]
  yellow-pro status [--profile NAME]
  yellow-pro disconnect [--profile NAME]
  yellow-pro setup claude-code|codex|gemini|cursor|hermes|openclaw|json
...
Env: YELLOW_PRO_BASE_URL, YELLOW_PRO_SANDBOX, YELLOW_PRO_API_KEY, YELLOW_PRO_API_SECRET,
     ...
     YELLOW_PRO_RATE_LIMIT_MS, YELLOW_PRO_PROFILE, YELLOW_PRO_CONFIG_PATH`;
...
function writeCursorConfig(file: string, env: Record<string, string>): void {
  ...
  if (existsSync(file)) {
    try {
      ...
      throw new YellowProError(`failed to read Cursor MCP config: ${String(error)}`);
    }
  }
  const currentServers = config.mcpServers;
  const servers = typeof currentServers === "object" && currentServers !== null && !Array.isArray(currentServers)
    ? currentServers as Record<string, unknown>
    : {};
  config.mcpServers = {
    ...servers,
    yellow_pro: { type: "stdio", command: "yellow-pro-mcp", args: [], env },
  };
  ...
  try {
    ...
    throw new YellowProError(`failed to write Cursor MCP config: ${String(error)}`);
  }
}
  ...
  switch (target) {
    ...
    case "codex": {
      ...
      try {
        ...
        const envToml = Object.entries(env).map(([key, value]) => `${key} = "${value}"`).join(", ");
        return `[mcp_servers.yellow_pro]\ncommand = "${command}"\nenv = { ${envToml} }`;
      }
    }
    case "gemini": {
      const flags = Object.entries(env).flatMap(([key, value]) => ["--env", `${key}=${value}`]);
      runner("gemini", ["mcp", "add", "--scope", "user", "--transport", "stdio", ...flags, "yellow-pro", command]);
      return "yellow_pro MCP server registered with Gemini CLI";
    }
    case "cursor": {
      const file = options.cursorConfigPath ?? join(homedir(), ".cursor", "mcp.json");
      writeCursorConfig(file, env);
      return `yellow_pro MCP server written to ${file}`;
    }
    case "hermes":
      try {
        const flags = Object.entries(env).flatMap(([key, value]) => ["--env", `${key}=${value}`]);
        runner("hermes", ["mcp", "add", "yellow_pro", "--command", command, ...flags], "y\n");
        return "yellow_pro MCP server registered with Hermes";
      } catch (error) {
        if (!allowFallback) {
          throw error;
        }
        const envYaml = Object.entries(env).map(([key, value]) => `      ${key}: "${value}"`).join("\n");
        return `mcp_servers:\n  yellow_pro:\n    command: ${command}\n    env:\n${envYaml}`;
      }
    case "openclaw": {
      const flags = Object.entries(env).flatMap(([key, value]) => ["--env", `${key}=${value}`]);
      runner("openclaw", ["mcp", "add", "yellow_pro", "--command", command, ...flags]);
      return "yellow_pro MCP server registered with OpenClaw";
    }
    case "json":
      return JSON.stringify({ mcpServers: { yellow_pro: { command, env } } }, null, 2);
    default:
      throw new YellowProError("setup target must be claude-code, codex, gemini, cursor, hermes, openclaw, or json");
  }
}
